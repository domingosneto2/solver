...
static bool runAntSolver2( double alpha, 
                          double beta, 
			  double minPheromone, 
			  double maxPheromone, 
			  int numAnts, 
			  ...
			  const char *filename )
{
	...
	cout << "Ant-Solver2 parameters:" << endl;
	cout << "\tRo:            " << ro << endl;
...
int benchmarkAntSolver2( int argc, char **argv )
{
	...
	res = runAntSolver2( alpha, beta, minPheromone, maxPheromone, numAnts, ro, numSteps, localSearch, filename );
	return res ? 0 : 1;
}
